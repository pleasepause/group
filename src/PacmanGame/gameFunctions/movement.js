...
export function getNewPosition(
  ...
  const trackHit = track.findIndex(
    limits =>
...
export function snapToTrack(plane, order, position, tolerance) {
  const snap = order ? Math.ceil(position[plane]) : Math.floor(position[plane]);
...
export function getChangedVector(
  ...
  if (!(old0 === new0 && movedDistanceBeforeTurn > movedDistance)) {
    const { order: newOrder, plane: newPlane, polarity } = orderPolarity(
      newDirection
    );
    ...
    const trackHit = track.findIndex(
      limits =>
        ...
        (1 - newOrder) * newPosition[newPlane] >=
          (limits[0] - polarity) * (1 - newOrder) &&
    ...
    if (trackHit > -1) {
      ...
      changedVector[newPlane] +=
        polarity * (movedDistance - movedDistanceBeforeTurn);
