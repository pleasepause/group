...
function getAvailableVectors({
  ...
  return availableOptions
    .map(direction => {
      const { polarity: optionPolarity, plane: optionPlane } = orderPolarity(
        direction
      );
      ...
      if (plane === optionPlane) {
        ...
        position = [];
        position[optionPlane] =
          newPosition[optionPlane] +
    ...
    .sort(
      ({ compare: posA }, { compare: posB }) =>
        (-1) ** (eating >> 0) *
        (gridDistance(posA, player.position) -
          gridDistance(posB, player.position))
    )
...
function getNextMonsterHomePosition(newPosition, monster, player) {
  if (
    (monster.direction === constants.EAST &&
  ...
  if (
    monster.direction === constants.NORTH &&
    ...
    return {
      ...
      direction:
        !monster.directionBias && monster.position[0] < player.position[0]
          ? constants.EAST
          : constants.WEST
    };
  }
...
function getIsHome(monster) {
  return (
    monster.position[0] > constants.MONSTER_HOME_RANGE[constants.WEST] &&
...
function getAvailableMonsterRoutes({
  newPosition,
  collision,
  plane,
  trackTo,
  ...
  if (trackTo !== -1 && tracks[1 - plane][trackTo]) {
    const passedTrack = tracks[1 - plane][trackTo].find(
      ([start, end]) =>
        newPosition[1 - plane] >= start && newPosition[1 - plane] <= end
    );
