...
function getLinePart({
  ...
  const vector =
    (radiusA + radiusB * (!end >> 0) - distance) * reverse * polarity;
  const line = `${vectorType}${vector}`;

  const cornerArcDiffX =
    radiusA *
    ((-1) ** ((lastDirection === constants.WEST) >> 0) * vertical -
      reverse * horizontal);

  const cornerArcDiffY =
    radiusA *
    ((-1) ** ((lastDirection === constants.NORTH) >> 0) * horizontal +
      reverse * vertical);

  const clockwise = (pmod(lastDirection - direction, 4) === 1) >> 0;

  const cornerArc = getCornerArc(
    radiusA,
    clockwise,
    cornerArcDiffX,
    cornerArcDiffY
...
function getPoint([xValue, yValue], gridSize) {
  return [xValue * gridSize, (constants.HEIGHT - yValue) * gridSize];
}
